...
impl ParsedShipData {
    ...
    */    if self.overall_height <= 0. {
        ...
        if self.bounds.is_empty() {
            return Err(Error::Parameter(
                "Error check ParsedShipData: bounds.is_empty()".to_string(),
            ));
        }
    ...
    */    if self.const_mass_shift_z < 0. {
        ...
        if self.center_waterline.is_empty() {
            return Err(Error::Parameter(
                "Error check ParsedShipData: center_waterline.is_empty()".to_string(),
            ));
        }
        if self.waterline_length.is_empty() {
            return Err(Error::Parameter(
                "Error check ParsedShipData: waterline_length.is_empty()".to_string(),
            ));
        }
        if self.waterline_breadth.is_empty() {
            return Err(Error::Parameter(
                "Error check ParsedShipData: waterline_breadth.is_empty()".to_string(),
            ));
        }
        if self.waterline_area.is_empty() {
            return Err(Error::Parameter(
                "Error check ParsedShipData: waterline_area.is_empty()".to_string(),
            ));
        }
        if self.volume_shift.is_empty() {
            return Err(Error::Parameter(
                "Error check ParsedShipData: volume_shift.is_empty()".to_string(),
            ));
        }
        if self.rad_long.is_empty() {
            return Err(Error::Parameter(
                "Error check ParsedShipData: rad_long.is_empty()".to_string(),
            ));
        }
        if self.rad_trans.is_empty() {
            return Err(Error::Parameter(
                "Error check ParsedShipData: rad_trans.is_empty()".to_string(),
            ));
        }
        if self.h_subdivision.is_empty() {
            return Err(Error::Parameter(
                "Error check ParsedShipData: h_subdivision.is_empty()".to_string(),
            ));
        }
        if self.mean_draught.is_empty() {
            return Err(Error::Parameter(
                "Error check ParsedShipData: mean_draught.is_empty()".to_string(),
            ));
        }
        if self.center_draught_shift.is_empty() {
            return Err(Error::Parameter(
                "Error check ParsedShipData: center_draught_shift_x.is_empty()".to_string(),
            ));
        }
        if self.pantocaren.is_empty() {
            return Err(Error::Parameter(
                "Error check ParsedShipData: pantocaren.is_empty()".to_string(),
            ));
        }
     /*   if let Some((draught, _)) = self.pantocaren.iter().find(|(draught, _)| *draught < 0.) {
        ...
        if self.flooding_angle.is_empty() {
            return Err(Error::Parameter(
                "Error check ParsedShipData: flooding_angle.is_empty()".to_string(),
            ));
        }
  /*      if let Some((key, value)) = self
        ...
        if self.entry_angle.is_empty() {
            return Err(Error::Parameter(
                "Error check ParsedShipData: entry_angle.is_empty()".to_string(),
            ));
        }
    /*    if let Some((key, value)) = self
            .entry_angle
        ...
        if self.frame_area.is_empty() {
            return Err(Error::Parameter(format!(
                "Error check ParsedShipData: frame_area.is_empty() {}",
                self.frame_area.len()
            )));
        }
        if let Some(frame) = self
            .frame_area
            .iter()
            .find(|f| f.immersion_area.iter().any(|v| v.1 < 0.))
        {
            return Err(Error::Parameter(format!(
                "Error check ParsedShipData: immersion_area frame{frame}: v.1 < 0"
            )));
        }
