    ...
    T: Value + Add<T, Output = T> + Sub<T, Output = T>,
{
    ...
    pub fn new(curves: Vec<(f64, Curve<T>)>) -> Result<Self, Error> {
        if curves.is_empty() {
            return Err(Error::new("Curve2D", "new").err("curves.is_empty()"));
        }
        Ok(Self { curves })
    }
    ...
    pub fn from_values_linear(values: &[(f64, Vec<(f64, T)>)]) -> Result<Self, Error> {
        if values.is_empty() {
            return Err(Error::new("Curve2D", "from_values_linear").err("values.is_empty()"));
        }
    ...
    pub fn from_values_catmull_rom(values: &[(f64, Vec<(f64, T)>)]) -> Result<Self, Error> {
        if values.is_empty() {
            return Err(Error::new("Curve2D", "from_values_catmull_rom").err("values.is_empty()"));
        }
        let mut curves = Vec::new();
        for (value, vector) in values.iter() {
            curves.push((
                *value,
                Curve::new_catmull_rom(vector).map_err(|e| {
                    Error::new("Curve2D", "from_values_catmull_rom")
                        .err(format!("{e}, value:{value}, vector:{:?}", vector))
                })?,
            ));
        }
        Self::new(curves)
    ...
    pub fn new_cosine(values: &[(f64, Vec<(f64, T)>)]) -> Result<Self, Error> {
        if values.is_empty() {
            return Err(Error::new("Curve2D", "new_cosine").err("values.is_empty()"));
        }
        let mut curves = Vec::new();
        for (value, vector) in values.iter() {
            curves.push((
                *value,
                Curve::new_cosine(vector).map_err(|e| {
                    Error::new("Curve2D", "from_values_catmull_rom")
                        .err(format!("{e}, value:{value}, vector:{:?}", vector))
                })?,
            ));
...
//
impl<T> ICurve2D<T> for Curve2D<T>
where
    T: Value + Add<T, Output = T> + Sub<T, Output = T>,
{
    ...
    fn value(&self, key1: f64, key2: f64) -> Result<CurveResult<T>, Error> {
        ...
        Ok(CurveResult::new(
            self.curves
                .last()
                .ok_or(Error::new("Curve2D", "value").err("no last curve"))?
                .1
                .value(key2)?,
            true,
        ))
