...
use crate::{algorithm::entities::{model_cached::ModelCached, Moment, Position, Position2d}, ship_model::query::{BulkData, LiquidData}};

use super::{local_cache::LocalCache};

//! Описание структур для расчета (равновесного положения)[https://github.com/a-givertzman/sss/blob/master/design/algorithm/part03_draft/chapter01_floatingPosition/chapter01_floatingPosition.md]
///
/// Структура для ввода данных расчета баланса судна.
/// Содержит массу судна, грузов и положение зерновых перегородок
#[derive(Debug, Clone)]
pub struct  FloatingPositionQuery {
...
//    pub damage_compartment: Vec<CompartmentResult>,
}
//
impl ModelCached {
    ///
    ...
    /*
        pub fn floating_position(&self) -> Result<EvaluatedFloatingPosition, Error> {
    ...
    */
    ...
    pub fn floating_position(&mut self, query: BalanceQuery) -> Result<EvaluatedFloatingPosition, Error> {
        let error = Error::new(&self.dbg, "eval");
