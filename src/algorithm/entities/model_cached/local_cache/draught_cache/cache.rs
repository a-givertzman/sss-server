...
impl DraughtCache {
    ...
    pub fn new(
        ...
        let dbg = Dbg::new(parent, "DraughtCache");
        let path = cache_dir.as_ref().join("draught_cache");
        Self {
            shape,
            heel_steps,
            trim_steps,
...
//
//
impl LocalCache for DraughtCache {
    //
    fn calculate(&mut self) -> Vec<Error> {
        let error = Error::new(&self.dbg, "calculate");
        let cache_data = super::build_cache::BuildDraughtCache::new(
            &self.dbg,
            self.shape.clone(),
