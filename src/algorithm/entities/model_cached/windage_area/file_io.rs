...
///
pub fn read(dbg: &Dbg, cache_path: &PathBuf) -> Result<Vec<(f64, Vec<(f64, f64)>)>, Error> {
    let error = Error::new(dbg, "read");
    let mut file = File::open(cache_path).map_err(|err| error.pass_with("File::open", err.to_string()))?;
    return match bincode::decode_from_std_read(&mut file, bincode::config::standard()) {
        Ok(data) => Ok(data),
        Err(err) => Err(error.pass_with("Encode error", err.to_string())),
    };
}
///
pub fn save(dbg: &Dbg, cache_path: &PathBuf, data: &Vec<(f64, Vec<(f64, f64)>)>) -> Result<(), Error> {
