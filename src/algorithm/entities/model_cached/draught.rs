...
impl Draught {
    ...
    pub fn calculate(&self) -> (f64, f64, f64) {
        ...
        let draught = |point: Vector3<f64>| {
            self.draught_mid + point.y * tg_theta + (point.x - self.midel_x) * tg_phi / cos_theta
        };
        let bow = Vector3::new(self.length_lbp, 0.0, -self.draught_mid);
        let stern = Vector3::new(0., 0.0, -self.draught_mid);
        let mean = Vector3::new(self.waterline_x, self.waterline_y, -self.draught_mid);
