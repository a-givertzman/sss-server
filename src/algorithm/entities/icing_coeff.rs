...
impl IIcingStab for IcingStab {
    ...
    fn mass_desc_h(&self) -> f64 {
        match self.icing_stab {
            IcingCoeffType::Full => self.icing_m_h_full,
            IcingCoeffType::Half => self.icing_m_h_half,
            _ => 0.,
        }
    }
    ...
    fn mass_timber_h(&self) -> f64 {
        match self.icing_stab {
            IcingCoeffType::Full | IcingCoeffType::Half => self.icing_m_timber,
            _ => 0.,
        }
    }
    ...
    fn mass_v(&self) -> f64 {
        match self.icing_stab {
            IcingCoeffType::Full => self.icing_m_v_full,
            IcingCoeffType::Half => self.icing_m_v_half,
            _ => 0.,
        }
    }
    ...
    fn coef_v_area(&self) -> f64 {
        match self.icing_stab {
            IcingCoeffType::Full => self.icing_coef_v_area_full,
            IcingCoeffType::Half => self.icing_coef_v_area_half,
    ...
    fn coef_v_moment(&self) -> f64 {
        match self.icing_stab {
            IcingCoeffType::Full => self.icing_coef_v_moment_full,
            IcingCoeffType::Half => self.icing_coef_v_moment_half,
            _ => self.icing_coef_v_moment_zero,
        }
    }
    /// Признак наличия обледенения
    fn is_some(&self) -> bool {
        matches!(self.icing_stab, IcingCoeffType::Full | IcingCoeffType::Half)
    }
}
//
#[doc(hidden)]
