    ...
    fn eval(&self, _: ()) -> EvalResult {
        match self.ctx.eval(()) {
            Ok(ctx) => {
                ...
                let vmax = initial_ctx.vessel_max_speed;
                ...
                let result: Vec<(f64, f64, f64)> = course_angle_of_wave
                    ...
                    .flat_map(|&angle| {
                        vessel_speeds
                            .iter()
                            .map(move |&speed| {
                                let apparent_frequency = 2.0 * std::f64::consts::PI * (3.0 * period_excitement + speed * (angle * std::f64::consts::PI / 180.0).cos()).abs() / (3.0 * period_excitement.powf(2.0));
                                (
                                    angle,
                                    speed,
                                    apparent_frequency
