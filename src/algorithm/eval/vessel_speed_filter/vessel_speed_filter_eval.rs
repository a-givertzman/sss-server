...
pub struct VesselSpeedFilterEval {
    dbg: Dbg,
    ctx: Box<dyn Eval<Zg, EvalResult> + Send + Sync>,
}
//
//
impl VesselSpeedFilterEval {
    ///
    /// Новый экземпляр [VesselSpeedFilterEval]
    pub fn new(parent: impl Into<String>, ctx: impl Eval<Zg, EvalResult> + Send + Sync + 'static) -> Self {
        let dbg = Dbg::new(parent, "VesselSpeedFilterEval");
        Self {
            dbg,
            ctx: Box::new(ctx),
        }
...
//
//
impl Eval<Zg, EvalResult> for VesselSpeedFilterEval {
    fn eval(&self, z_g_fix: Zg) -> EvalResult {
        let error = Error::new(&self.dbg, "eval");
        match self.ctx.eval(z_g_fix) {
            Ok(ctx) => {
                let main_resonant_zone = ContextRead::<MainResonantZoneCtx>::read(&ctx).clone();
                ...
                for freq in apparent_frequencies {
                    if (main_resonant_zone.left_side <= freq.1 && freq.1 <= main_resonant_zone.right_side) 
                    && (parametric_resonant_zone.left_side <= freq.1 && freq.1 <= parametric_resonant_zone.right_side) {
                        result.push(freq.1);
                    }
                }
                ctx.write(
...
//
//
impl std::fmt::Debug for VesselSpeedFilterEval {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("VesselSpeedFilterEval")
            .field("dbg", &self.dbg)
            .finish()
    }
}
