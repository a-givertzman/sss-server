...
use crate::{
    ...
    algorithm::{
        ...
        eval::{CriterionStabilityCtx, MetacentricHeightEval, parameters::ParameterID},
    },
...
pub struct ZgEval {
    dbg: Dbg,
    scheduler: Scheduler,
    ctx_before: MetacentricHeightEval,
    ctx_after: Box<dyn Fn(MetacentricHeightEval) -> EvalResult>,
}
//
//
impl ZgEval {
    ...
    pub fn new(
        ...
        ctx_before: MetacentricHeightEval,
        ctx_after: impl Fn(MetacentricHeightEval) -> EvalResult + 'static,
    ) -> Self {
...
impl Eval<(), EvalResult> for ZgEval {
    fn eval(&mut self, _: ()) -> EvalResult {
        ...
        let base_task = self
            .scheduler
            .spawn(|| {
                base_ctx.write((self.ctx_after)(self.ctx_before)?);
                Ok(())
            })
        ...
        let overall_height = *ship_parameters
            ...
            .ok_or(CtxResult::Err(error.err("No LBP in ship_parameters")))?;
