use crate::{
    algorithm::{
        ...
        eval::{
            Zg,
            import_model::{
                convert_to_trimesh::{ctx::ConvertToTrimeshCtx, eval::ConvertToTrimeshEval},
...
#[test]
fn convert_tanks_to_trimesh_fixed() {
    ...
    let path_3d_model = "src\\tests\\unit\\algorithm\\dialog_static\\test_files\\unboxes_АРК_2023";
    ...
    let result = ConvertToTrimeshEval::new("Test", ImportModelInitialPointsEval::new("Test", ctx))
        .eval(Zg::empty());
    match result {
        Ok(ctx) => {
            let result: ConvertToTrimeshCtx = ctx.read();
            match result.result {
                Some(mesh) => {
