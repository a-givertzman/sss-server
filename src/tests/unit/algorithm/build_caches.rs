...
use crate::conf::Conf;
use crate::kernel::{run::Run, types::Arc};
...
fn build_caches() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let compartments_max: HashMap<String, (Option<f64>, f64)> = compartments_max
        .into_iter()
        .map(|(code, level, volume)| (code.to_owned(), (Some(level), volume)))
        .collect();
